...
const { TickMath , FullMath, computePoolAddress, Pool, Position, nearestUsableTick, MintOptions, NonfungiblePositionManager, AddLiquidityOptions, RemoveLiquidityOptions, CollectOptions,FeeAmount} = require('@uniswap/v3-sdk');
...
const currentPoolAddress = computePoolAddress({
  factoryAddress: "0x88e6A0c2dDD26FEEb64F039a2c41296FcB3f5640",
  tokenA: USDC_TOKEN ,
  tokenB: WETH_TOKEN,
  fee: FeeAmount.LOW,
});
...
async function main (baseToken, quoteToken) {

  const inputAmount = await weth.balanceOf(impersonatedSigner.address);
...
const positionId = await getPositionIds();
  ...
  usdcAmount = await weth.balanceOf(impersonatedSigner.address);
  wethAmount = await usdc.balanceOf(impersonatedSigner.address);
  ...
  await impersonatedSigner.sendTransaction(transaction);
}
...
async function removeLiquidity( )
  ...
  return new Promise((resolve, reject) => {
    resolve(impersonatedSigner.sendTransaction(transaction));
...
main(baseToken, quoteToken, 1, 201659, 18, 6)